...
using DelegateCommand = OATCommunications.Avalonia.DelegateCommand;
...
#pragma warning restore CS0414
		...
		string _lastSlewCommandStatus = string.Empty;
		...
		private string _poiFile = string.Empty;
		...
		private string _serialBaudRate = string.Empty;

		private OatmealTelescopeCommandHandlers? _oatMount;
		private PointsOfInterest _pointsOfInterest = new PointsOfInterest();
		...
		private string _incrVar = string.Empty;
		...
		private global::Avalonia.Controls.Window GetDialogOwner()
		{
			...
			throw new InvalidOperationException("No dialog owner window is available.");
		}
		...
		public void OnMonitorSharpCapChanged(bool oldVal, bool newVal)
		{
			AppSettings.Instance.MonitorSharpCapPA = newVal;
			AppSettings.Instance.Save();
		...
		public MountVM()
		{
				...
				try
				{
					...
					for (int i = 0; i < types.Length; i++)
					{
						...
						if (type?.GetInterface("ICommunicationHandler") != null)
						{
							...
							if (obj is ICommunicationHandler handler)
							{
								CommunicationHandlerFactory.AddHandler(handler);
			...
			_timerStatus = new DispatcherTimer();
			_timerStatus.Interval = TimeSpan.FromMilliseconds(500);
			_timerStatus.Tick += async (s, e) => await OnTimer(s!, e);
			_timerStatus.Start();
			_timerFineSlew = new DispatcherTimer();
			_timerFineSlew.Interval = TimeSpan.FromMilliseconds(200);
			_timerFineSlew.Tick += async (s, e) => await OnFineSlewTimer(s!, e);
			...
			_driftAlignCommand = new DelegateCommand(async dur => await OnRunDriftAlignment(int.Parse(dur?.ToString() ?? "0")), () => MountConnected);
			...
			_showMiniControllerCommand = new DelegateCommand(() =>
			{
				Log.WriteLine("UI: ShowMiniController command invoked");
				OnShowMiniController();
			}, () => true);
			...
			if (!File.Exists(_listFilePath))
			{
				...
				try
				{
					...
					{
						var parent = System.IO.Path.GetDirectoryName(System.IO.Path.GetDirectoryName(_listFilePath));
						if (parent == null) break;
						_listFilePath = System.IO.Path.Combine(parent, "Checklist.txt");
					}
				}
				catch
				{
					_listFilePath = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "OpenAstroTracker", "Checklist.txt");
					Directory.CreateDirectory(System.IO.Path.GetDirectoryName(_listFilePath)!);
		...
		public async void OnShowAppSettings()
		{
			DlgAppSettings dlg = new DlgAppSettings(this) { WindowStartupLocation = global::Avalonia.Controls.WindowStartupLocation.CenterOwner };
			await dlg.ShowDialog<object?>(GetDialogOwner());
		}
		...
		public void OnRunStepCalibration()
		{
			...
			DlgAxisCalibration dlg = new DlgAxisCalibration(this) { WindowStartupLocation = global::Avalonia.Controls.WindowStartupLocation.CenterScreen };
			dlg.Show();
		}
		...
		public void OnCustomAction(string nr)
		{
			...
			// Keyboard modifier check not available in Avalonia VM context.
			// Custom action configuration needs a separate UI path.
			if (!string.IsNullOrEmpty(_customCommandText[cmdNr]))
			{
		...
		private void OnStartChangingParameter(object p)
		{
			if (_timer == null)
			{
				...
				_timer.Interval = TimeSpan.FromMilliseconds(InitialIncrementalDelay);
				_timer.Tick += OnIncrementVariable;
			}

			string? par = p as string;
			if (par == null || par.Length < 4) return;
			if (par[0] == '+')
			{
		...
		{
			// Confirm before wiping mount settings
			var confirmDlg = new DlgMessageBox("This will reset all mount settings to factory defaults.\n\nContinue?");
			var result = await confirmDlg.ShowDialog<bool?>(GetDialogOwner());
			if (result != true)
			{
				return;
			}
			...
			var msgDlg = new DlgMessageBox(message);
			await msgDlg.ShowDialog<bool?>(GetDialogOwner());
		}
		...
		private async Task OnShowTargetChooser()
		{
			if (_targetChooser == null)
			{
				...
				if (AppSettings.Instance.TargetChooserPos.X != -1)
				{
					...
					_targetChooser.Width = AppSettings.Instance.TargetChooserSize.W;
					_targetChooser.Height = AppSettings.Instance.TargetChooserSize.H;
				}
			}
		...
		private void OnShowMiniController()
		{
			...
			const double miniOffset = 12;

			if (_miniController == null)
			{
				Log.WriteLine("MOUNT: Creating MiniController window instance");
				_miniController = new MiniController(this);
				_miniController.Topmost = KeepMiniControlOnTop;

				if (mainWindow != null)
				{
					_miniController.Left = mainWindow.Position.X + miniOffset;
					_miniController.Top = mainWindow.Position.Y + miniOffset;
				}
				else
				{
					var r = AppSettings.Instance.EnsureRectIsOnScreen("MiniControllerPos", null);
			...
			{
				Log.WriteLine("MOUNT: Hiding MiniController");
			...
			else
			{
				...
				Log.WriteLine("MOUNT: Showing MiniController");
				_miniController.Show();
				_miniController.Activate();
				_miniController.Focus();
			}
		}
		...
		private async void OnShowSettingsDialog()
		{
			if (_dlgMountSettings != null)
			{
				_dlgMountSettings.Activate();
				return;
			}
			...
			_dlgMountSettings = dialog;
			dialog.Closed += (_, _) => _dlgMountSettings = null;
			await dialog.ShowDialog<object?>(GetDialogOwner());
		}
		...
		{
			// Keyboard state check removed for Avalonia compatibility
							...
							if (result.Success)
							{
								...
								if (result.Success && !string.IsNullOrWhiteSpace(status))
								{
									try
									{
										...
										bool prevTracking = _isTracking;
										_isGuiding = false;
										MountStatus = parts[0];
										SyncParkCommandFromStatus(MountStatus);
										Log.WriteLine("UPDATE: GX status transition {0} -> {1}, parkButton={2}, trkFlag={3}", prevStatus, MountStatus, ParkCommandString, parts[1].Length > 2 ? parts[1][2].ToString() : "?");
										...
										if (prevTracking != _isTracking)
										{
											Log.WriteLine("UPDATE: Tracking state from GX changed {0} -> {1} (status={2})", prevTracking, _isTracking, MountStatus);
										}
		...
		private async Task OnPark()
		{
			if (ParkCommandString == "Park")
			{
				...
				{
					Log.WriteLine("MOUNT: Park command reply success={0}, data=[{1}]", a.Success, a.Data);
			...
			else
			{
				...
				{
					Log.WriteLine("MOUNT: Unpark command reply success={0}, data=[{1}]", a.Success, a.Data);
		...
		private void SyncParkCommandFromStatus(string status)
		{
			ParkCommandString = string.Equals(status, "Parked", StringComparison.OrdinalIgnoreCase)
				? "Unpark"
				: "Park";
		}
		...
		private async Task OnStopSlewing(char dir)
		{
			...
			void UpdateSlewStatus(string commandSent, CommandResponse response)
			{
				string status = response.Success ? "OK" : "FAIL";
				string detail = response.Success ? response.Data : response.StatusMessage;
				LastSlewCommandStatus = $"{DateTime.Now:HH:mm:ss} {commandSent} {status} {detail}".Trim();
			}
			...
			if ((dir != 'a') && (dir != 'z') && (dir != 'l') && (dir != 'r'))
			{
				...
				if ((dir == 'f') || (dir == 'g'))
				{
					...
					if (ScopeHasFOC)
					{
						Log.WriteLine("CONTROL: Sending stop focuser command [:FQ#]");
						_oatMount?.SendCommand(":FQ#", (a) => { Log.WriteLine("CONTROL: Stop focuser reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(":FQ#", a); doneEvent.Set(); });
					}
					else
					{
						doneEvent.Set();
				...
				else
				{
					if (dir == '*')
					{
						...
						{
							Log.WriteLine("CONTROL: Sending stop focuser command [:FQ#]");
							_oatMount?.SendCommand(":FQ#", (a) => { Log.WriteLine("CONTROL: Stop focuser reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(":FQ#", a); });
						}
						Log.WriteLine("CONTROL: Sending stop-all command [:Q#]");
						_oatMount?.SendCommand(":Q#", (a) => { Log.WriteLine("CONTROL: Stop-all reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(":Q#", a); doneEvent.Set(); });
					}
					else
					{
						// Slew commands
						var cmd = $":Q{dir}#";
						Log.WriteLine("CONTROL: Sending axis stop command [{0}]", cmd);
						_oatMount?.SendCommand(cmd, (a) => { Log.WriteLine("CONTROL: Axis stop reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(cmd, a); doneEvent.Set(); });
					}
					await RecalculatePointsPositions(true);
				}
			}
		...
		public bool InvertALTCorrections
		{
			get
			{
				return _invertAltCorrections;
			}
			set
			{
				if (_invertAltCorrections != value)
				{
		...
		public bool InvertAZCorrections
		{
			get
			{
				return _invertAzCorrections;
			}
			set
			{
				if (_invertAzCorrections != value)
				{
		...
		private async Task OnStartSlewing(char dir)
		{
			...
			if ((dir == 'a') || (dir == 'z'))
			{
				var cmd = string.Format(_oatCulture, ":MAL{0}{1:0.0}#", sign, distance);
				Log.WriteLine("CONTROL: Sending ALT slew command [{0}]", cmd);
				_oatMount?.SendCommand(cmd, (a) => { Log.WriteLine("CONTROL: ALT slew reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(cmd, a); doneEvent.Set(); });
			}
			else if ((dir == 'l') || (dir == 'r'))
			{
				var cmd = string.Format(_oatCulture, ":MAZ{0}{1:0.0}#", sign, distance);
				Log.WriteLine("CONTROL: Sending AZ slew command [{0}]", cmd);
				_oatMount?.SendCommand(cmd, (a) => { Log.WriteLine("CONTROL: AZ slew reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(cmd, a); doneEvent.Set(); });
			}
			else if ((dir == 'f') || (dir == 'g'))
			{
				if (ScopeHasFOC)
				{
					var rateCmd = string.Format(_oatCulture, ":F{0}#", SlewRate);
					var focCmd = string.Format(_oatCulture, ":F{0}#", sign);
					Log.WriteLine("CONTROL: Sending FOC rate command [{0}]", rateCmd);
					_oatMount?.SendCommand(rateCmd, (a) => { Log.WriteLine("CONTROL: FOC rate reply success={0}, data=[{1}]", a.Success, a.Data); });
					Log.WriteLine("CONTROL: Sending FOC slew command [{0}]", focCmd);
					_oatMount?.SendCommand(focCmd, (a) => { Log.WriteLine("CONTROL: FOC slew reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(focCmd, a); doneEvent.Set(); });
				}
				else
				{
					doneEvent.Set();
			...
			{
				var cmd = string.Format(_oatCulture, ":M{0}#", dir);
				Log.WriteLine("CONTROL: Sending RA/DEC slew command [{0}]", cmd);
				_oatMount?.SendCommand(cmd, (a) => { Log.WriteLine("CONTROL: RA/DEC slew reply success={0}, data=[{1}]", a.Success, a.Data); UpdateSlewStatus(cmd, a); doneEvent.Set(); });
			}
			await doneEvent.WaitAsync();
		...
		private async Task OnChangeSlewingState(string direction)
		{
			...
			{
				LastSlewCommandStatus = $"{DateTime.Now:HH:mm:ss} Sending {direction}";
		...
		private async Task OnSlewToTarget()
		{
			...
			if (ShowDECLimits && _slewTargetValid)
			{
				...
				if (lowerLimitExceeded || upperLimitExceeded)
				{
					...
					var dlg = new DlgMessageBox($"This target is {violatedConstraint} currently set DEC limit by {overBy:0.0} degrees.\n\nSlew anyway?");
					var result = await dlg.ShowDialog<bool?>(GetDialogOwner());
					if (result != true)
					{
						return;
					}
		...
		public void Disconnect()
		{
			...
			if (_commHandler != null)
			{
				...
				{
					Log.WriteLine("MOUNT: Exception during disconnect (ignored): {0}", ex.Message);
				}
				_commHandler = null;
			}
			...
			ScopeType = "OAT";
			ConnectionState = string.Empty;
			LastSlewCommandStatus = string.Empty;
			ScopeHardware = string.Empty;
			MountStatus = string.Empty;
			ParkCommandString = "Park";
			_oatMount = null;
			RequeryCommands();
		}
		...
		public async Task UpdateInitialScopeValues()
		{
			...
			catch (FormatException fex)
			{
				...
				var dlg = new DlgMessageBox("Connected to OpenAstroTracker, but protocol could not be established.\n\nIs the firmware compiled with DEBUG_LEVEL set to DEBUG_NONE?");
				dlg.Show();
			}
			catch (Exception ex)
			{
				...
				var dlg = new DlgMessageBox("Error trying to connect to OpenAstroTracker.\n\n" + ex.Message);
				dlg.Show();
			}
		}
		...
		private async void OnConnectToTelescope()
		{
			...
			{
				Log.WriteLine("MOUNT: Unhandled exception while connecting: {0}", ex);
				Disconnect();
				var dlg = new DlgMessageBox("Error trying to connect to OpenAstroTracker.\n\n" + ex.Message);
				dlg.Show();
			}
		...
		{
			var lastDevice = AppSettings.Instance.LastConnectedDevice;
			if (string.IsNullOrEmpty(lastDevice))
			{
				return false;
			}

			Log.WriteLine("MOUNT: Auto-connect attempt on last device [{0}]", lastDevice);
			_serialBaudRate = AppSettings.Instance.BaudRate;

			var result = await ConnectToOat(lastDevice);
			if (!result.Item1)
			{
				Log.WriteLine("MOUNT: Auto-connect failed: {0}", result.Item2);
				return false;
			}

			Log.WriteLine("MOUNT: Auto-connect succeeded, initialising scope");
			_inStartup = true;
			try
			{
				await UpdateInitialScopeValues();
				await UpdateStatus();
				UpdateLocation();
				await SendLocationToMount(AppSettings.Instance.SiteLatitude, AppSettings.Instance.SiteLongitude);
			}
			catch (Exception ex)
			{
				Log.WriteLine("MOUNT: Auto-connect initialisation failed: {0}", ex.Message);
				Disconnect();
				_inStartup = false;
				return false;
			}
			...
			_timerStatus.Start();
			_inStartup = false;
			Log.WriteLine("MOUNT: Auto-connect complete");
			return true;
		}

		private async Task SendLocationToMount(float latitude, float longitude)
		{
			var latDone = new AsyncAutoResetEvent();
			var lonDone = new AsyncAutoResetEvent();

			char latSign = latitude < 0 ? '-' : '+';
			float absLat = Math.Abs(latitude);
			int latDeg = (int)absLat;
			int latMin = (int)((absLat - latDeg) * 60.0f);
			SendOatCommand($":St{latSign}{latDeg:00}*{latMin:00}#,n", _ => latDone.Set());

			float fwLongitude = (_firmwareVersion < 11105) ? (180.0f - longitude) : (-longitude);
			char lonSign = fwLongitude < 0 ? '-' : '+';
			float absLon = Math.Abs(fwLongitude);
			int lonDeg = (int)absLon;
			int lonMin = (int)((absLon - lonDeg) * 60.0f);
			string lonCmd = (_firmwareVersion < 11105)
				? $":Sg{lonDeg:000}*{lonMin:00}#,n"
				: $":Sg{lonSign}{lonDeg:000}*{lonMin:00}#,n";
			SendOatCommand(lonCmd, _ => lonDone.Set());

			await latDone.WaitAsync();
			await lonDone.WaitAsync();
			Log.WriteLine("MOUNT: Auto-connect: location sent lat={0}, lon={1}", latitude, longitude);
		}
		...
		private void OnRunPolarAlignment()
		{
			...
			DlgRunPolarAlignment dlg = new DlgRunPolarAlignment(this.SendOatCommand) { WindowStartupLocation = global::Avalonia.Controls.WindowStartupLocation.CenterScreen };
			dlg.Show();
		...
		public async Task<Tuple<bool, string>> ConnectToOat(string device)
		{
			...
			string BuildFriendlyConnectFailureMessage(string rawMessage)
			{
				string friendly = string.IsNullOrWhiteSpace(rawMessage)
					? "Unable to connect to OpenAstroTracker."
					: rawMessage.Trim();

				bool isSerial = device.StartsWith("Serial", StringComparison.OrdinalIgnoreCase);
				if (isSerial && !friendly.Contains("in use", StringComparison.OrdinalIgnoreCase)
					&& !friendly.Contains("Unable to open", StringComparison.OrdinalIgnoreCase)
					&& !friendly.Contains("port", StringComparison.OrdinalIgnoreCase))
				{
					friendly += " Serial port possibly in use by another application.";
				}
			...
			if (_commHandler != null)
			{
				...
				{
					message = "Unable to open communication device.";
					if (device.StartsWith("Serial", StringComparison.OrdinalIgnoreCase))
					{
						message += " Serial port possibly in use by another application.";
					}
					failed = true;
				}
			}
			...
			{
				message = $"Unable to create communication handler for '{device}'.";
				failed = true;
			}
			...
			if (failed)
			{
				var friendlyMessage = BuildFriendlyConnectFailureMessage(message);
				ConnectionState = $"Connection failed: {friendlyMessage}";
				Log.WriteLine("MOUNT: {0}", ConnectionState);
				return Tuple.Create(false, friendlyMessage);
			}
			...
			if (hwParts.Length < 3)
			{
				message = $"Unexpected hardware response '{hwData}'";
				Log.WriteLine("MOUNT: {0}", message);
				_commHandler?.Disconnect();
				var friendlyMessage = BuildFriendlyConnectFailureMessage(message);
				ConnectionState = $"Connection failed: {friendlyMessage}";
			...
			if (raParts.Length < 2 || decParts.Length < 2)
			{
				message = $"Unexpected motor metadata in hardware response '{hwData}'";
				Log.WriteLine("MOUNT: {0}", message);
				_commHandler?.Disconnect();
				var friendlyMessage = BuildFriendlyConnectFailureMessage(message);
				ConnectionState = $"Connection failed: {friendlyMessage}";
			...
			if (_firmwareVersion > 10875)
			{
				...
				if (steppers.Length >= 2)
				{
					...
					if (raStepper.Length >= 3 && decStepper.Length >= 3)
					{
						switch (raStepper[0])
						{
					...
					{
						Log.WriteLine("MOUNT: Unexpected stepper metadata '{0}'", stepperData);
					}
				}
				else
				{
					Log.WriteLine("MOUNT: Unexpected stepper response '{0}'", stepperData);
				}
			}
		...
		private async Task<bool> ChooseTelescope()
		{
			...
			var connectVM = new ConnectVM(SendOatCommand,
				AppSettings.Instance.SiteLatitude,
				AppSettings.Instance.SiteLongitude,
				AppSettings.Instance.SiteAltitude,
				AppSettings.Instance.RunAutoHomeRAOnConnect,
				AppSettings.Instance.RunDECOffsetHomingOnConnect);
			connectVM.ConnectToOatFunc = async (deviceName) =>
			{
				var deviceToConnect = deviceName;
				if (deviceName.StartsWith("Serial", StringComparison.OrdinalIgnoreCase) && !deviceName.Contains("@"))
				{
					deviceToConnect = $"{deviceName}@{_serialBaudRate}";
				}

				Log.WriteLine("MOUNT: Wizard connect attempt on {0}", deviceToConnect);
				var connectResult = await ConnectToOat(deviceToConnect);
				if (connectResult.Item1)
				{
					AppSettings.Instance.LastConnectedDevice = deviceToConnect;
					AppSettings.Instance.Save();
				}
				return (connectResult.Item1, connectResult.Item2);
			};
			dlg.SetViewModel(connectVM);
			...
			if (connectVM.Result == true)
			{
				...
				if (dlg.RunDECOffsetHoming && (decOffset != 0) && !ScopeHasHSAV)
				{
					...
					setHome = dlgWait.DialogResult ?? false;
					abortHoming = !(dlgWait.DialogResult ?? false);
					if (!(dlgWait.DialogResult ?? false))
					{
			...
			else if (connectVM.Result == null || connectVM.Result == false)
			{
				Log.WriteLine("MOUNT: Chooser cancelled");

				// If the transport was already opened by the wizard (e.g. cancel during GPS wait),
				// make sure we fully disconnect before returning.
				if (MountConnected || _commHandler != null || _oatMount != null)
				{
					Disconnect();
				}
		...
		public bool IsMiniControllerVisible => _miniController?.IsVisible == true;

		public bool HandleMiniControllerKeyDown(Key key)
		{
			if (!MountConnected)
			{
				return false;
			...
			string cmdParam = key switch
			{
				Key.Up => "+N",
				Key.Down => "+S",
				Key.Left => "+W",
				Key.Right => "+E",
				Key.W when ScopeHasALT => "+A",
				Key.S when ScopeHasALT => "+Z",
				Key.A when ScopeHasAZ => "+L",
				Key.D when ScopeHasAZ => "+R",
				Key.X when ScopeHasFOC => "+F",
				Key.C when ScopeHasFOC => "+G",
				_ => string.Empty
			};
		...
		public bool HandleMiniControllerKeyUp(Key key)
		{
			if (key == Key.Escape)
			{
			...
			{
				case Key.H:
					if (_homeCommand.CanExecute(null))
					{
						_homeCommand.Execute(null);
						return true;
					}
					return false;
				case Key.P:
					if (_parkCommand.CanExecute(null))
					{
						_parkCommand.Execute(null);
						return true;
					}
					return false;
				case Key.D1:
		...
		public string LastSlewCommandStatus
		{
			get { return _lastSlewCommandStatus; }
			set { SetPropertyValue(ref _lastSlewCommandStatus, value); }
		}
		...
		public bool MonitorSharpCapForPA
		{
			...
			set { SetPropertyValue(ref _monitorSharpCapForPA, value, OnMonitorSharpCapChanged); }
		}
		...
		{
			get { return AppSettings.Instance.NinaLogFolder; }
			set
			{
				if (AppSettings.Instance.NinaLogFolder != value)
				{
					AppSettings.Instance.NinaLogFolder = value;
					AppSettings.Instance.Save();
				}
			}
		}
		...
		{
			get { return AppSettings.Instance.SharpCapLogFolder; }
			set
			{
				if (AppSettings.Instance.SharpCapLogFolder != value)
				{
					AppSettings.Instance.SharpCapLogFolder = value;
					AppSettings.Instance.Save();
				}
			}
		}
		...
		{
			Log.WriteLine("MOUNT: Tracking toggle requested old={0}, new={1}, MountStatus={2}, ParkButton={3}", oldVal, newVal, MountStatus, ParkCommandString);
			if (MountConnected)
			{
				try
				{
					Task.Run(async () =>
					{
						var trackingResult = await _oatMount!.SetTracking(newVal);
						Log.WriteLine("MOUNT: Tracking command completed result=[{0}] requested={1}", trackingResult, newVal);
					});
				}
				catch (Exception ex)
				{
					Debug.WriteLine("Unable to set Tracking mode." + ex.Message);
					Log.WriteLine("MOUNT: Tracking command exception {0}", ex.Message);
				}
			}
		}
		/// <summary>
		...
		private string _autoHomeRaDirection = "East";
		private float _autoHomeRaDistance;
		private string _autoHomeDecDirection = "South";
		...
		public bool KeepSlewToPointControlOnTop
		{
			...
			set
			{
				...
				if (_miniController != null && _slewPointsWindow != null)
				{
