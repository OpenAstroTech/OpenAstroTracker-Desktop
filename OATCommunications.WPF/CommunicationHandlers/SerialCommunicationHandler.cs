...
namespace OATCommunications.WPF.CommunicationHandlers
{
	public class SerialCommunicationHandler : CommunicationHandler
	{
		...
		protected override void RunJob(Job job)
		{
			...
			if (Connected)
			{
				long extraBytes = _port.BytesToRead;
				if (_logJobs)
				{
					if (extraBytes > 0) 
					{
						Log.WriteLine("SERIAL: {0:0000}: [{1}] Discarded {2} bytes in input buffer!", job.Number, job.Command, extraBytes);
					}
				}
				_port.DiscardInBuffer();
				try
				{
					if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Sending command", job.Number, job.Command);
				...
				try
				{
					switch (job.ResponseType)
					{
						...
						{
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] No response needed for command", job.Number, job.Command);
						...
						{
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Expecting single digit response for command, waiting...", job.Number, job.Command);
							string responseStr = new string((char)_port.ReadChar(), 1);
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Received single digit response '{2}' for command", job.Number, job.Command, responseStr);
						...
						{
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Expecting #-delimited response for Command, waiting...", job.Number, job.Command);
							string responseStr = _port.ReadTo("#");
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Received response '{2}' for command", job.Number, job.Command, responseStr);
							response = new CommandResponse(responseStr, true);
						}
						break;
						case ResponseType.DoubleFullResponse:
						{
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Expecting two #-delimited responses for Command, waiting for first...", job.Number, job.Command);
							string responseStr = _port.ReadTo("#");
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Received first response '{2}' for command", job.Number, job.Command, responseStr);
							response = new CommandResponse(responseStr, true);
							responseStr = _port.ReadTo("#");
							if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: [{1}] Received second response '{2}' for command, ignoring", job.Number, job.Command, responseStr);
						}
						break;
					}
				}
				...
				{
					Log.WriteLine("SERIAL: {0:0000}: [{1}] Failed to receive response to command. {2}", job.Number, job.Command, ex.Message);
			...
			if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: Calling OnFulFilled lambda", job.Number);
			job.OnFulFilled(response);
			job.Succeeded = response.Success;
			if (_logJobs) Log.WriteLine("SERIAL: {0:0000}: Job completed", job.Number);
		}
